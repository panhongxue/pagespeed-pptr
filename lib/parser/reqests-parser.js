...
class RequestsParser {
  ...
  get TTFB() {
    ...
    const timing = timings[0].timing
    const start = Math.max(timing.sendEnd, timing.connectEnd, timing.dnsEnd, timing.proxyEnd)
    const end = timing.receiveHeadersEnd
    return prettyMilliseconds(end - start, { locale: 'zh_CN' })
  }

  get summary() {
    return `${this.requests.length} 个请求    ${prettyBytes(
      this.resourceSize
    )} 项资源    完成用时：${this.requestTime}    TTFB：${this.TTFB}`
  }
}
